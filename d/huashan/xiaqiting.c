...
//Make some Change By Yanyan For Dtsl
//Make some rules for playing
...
object check_player=0;

void create ()
{
  set ("short","下棋亭");
  set ("long", @LONG
“棋亭”又名“博台”，传说是秦昭王令工施钧梯上华山，以松柏
之心为博箭，长八尺，棋长八寸，而勒之曰，王与天神博于此，故谓为
卫叔卿之“博台”。
LONG);
...
int give_reward(object win,object loser,int flag)
{
   if(!flag)
    flag=1;
   
   if(status ==WQ_PLAYING_WUZI){
     if(win->query("xiaqi/wuziqi")-loser->query("xiaqi/wuziqi")>10){
       loser->add("xiaqi/wuziqi",-1*flag);
       return 1;
     }
     win->add("xiaqi/wuziqi",1*flag);
     loser->add("xiaqi/wuziqi",-1*flag);
     return 1;
  }
     
  if(win->query("xiaqi/weiqi")-loser->query("xiaqi/weiqi")>10){
       loser->add("xiaqi/weiqi",-1*flag);
       return 1;
     }
     win->add("xiaqi/weiqi",1*flag);
     loser->add("xiaqi/weiqi",-1*flag);
     return 1; 
}
...
int do_check()
{
  ...
  tell_object(ob,"你目前五子棋总共胜了"+chinese_number(ob->query("xiaqi/wuziqi"))+"盘，\n"+
                 "      五子棋的等级为"+HIG+chinese_number((ob->query("xiaqi/wuziqi")>0?ob->query("xiaqi/wuziqi"):0)/100)+NOR"段。\n\n"+
                 "      围期总共胜了"+chinese_number(ob->query("xiaqi/weiqi"))+"盘，\n"+
                 "      围棋的等级为"+HIG+chinese_number((ob->query("xiaqi/weiqi")>0?ob->query("xiaqi/weiqi"):0)/100)+NOR"段。\n");
  return 1;
}
...
int do_leave(string arg)
{
	...
	if(objectp(pl[p])){
	 message_vision("$N此局输了！\n",me);
	give_reward(pl[p],me,5);
	}
	map_delete(pl,s);
...
int do_new(string arg)
{
	...
	tell_room(rm,show_game());
        player = pl[turn];
        if(turn=="black")
           message_vision("现在轮到黑方$N走棋。\n",player);
          else message_vision("现在轮到白方$N走棋。\n",player);
        remove_call_out("check_xiaqi");
        check_player=player;
        call_out("check_xiaqi",60,player);
	return(1);
}

void check_xiaqi(object player)
{
   if(!player) return;
   if(check_player==player)
    message_vision("$N出棋太慢，此局输了！\n",player);
   if(pl["white"]==player) give_reward(pl["black"],player,1);
   else give_reward(pl["white"],player,1);
   status=WQ_NOT_PLAYING;
}
...
int do_surrender()
{
   ...
   if(!ob->query_temp("weiqi_seat"))
   return notify_fail("你是旁观者，怎么好意思瞎指挥？\n");
   if(status==WQ_NOT_PLAYING) 
   return notify_fail("还没新开一局棋呐。\n");
   
   if(ob==pl["white"]) op=pl["black"];
   else op=pl["white"];
   
   message_vision("$N向$n认输！\n",ob,op);
   remove_call_out("check_xiaqi");
   give_reward(op,ob,1);
   check_player=0;
   status=WQ_NOT_PLAYING;
...
{
  if(ob->query_temp("weiqi_seat"))
  return notify_fail("你正在下棋呢，要离开用 leave 吧。\n");
