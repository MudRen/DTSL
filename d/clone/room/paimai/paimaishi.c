...
object restore_saleload(object owner,string basename)
{
	...
	for(i=0; i<sizeof(saleload); i++) {
		...
		if(file==basename){
		  ob = new(file);
		  if( !ob ) {
			tell_object(owner,"你拍卖的东西丢失了，最好通知一下巫师。\n");
...
void create()
{
		...
		set("title", HIW "鉴定专家" NOR);
		set("nickname", HIW "大唐第一锤" NOR);
        set("gender", "男性");
        set("age", 65);
        set("no_get", 1);
        set("long",
                "这是一位瘦高的白发老人，因为公正无私而被推为大唐第一拍卖师。\n");
...
int do_paimai(string arg)
{
	...
	command("chat "+"有人提交"+NOR+ob->query("name")+HIC+"进行拍卖，请使用"+HIG"【拍卖】"NOR+HIC"频道（sale）了解具体情况。"NOR);
...
int report_paimai(object buyer,int money,int num)
{
	...
	else
	{
       ...
       command("sale  有人出价 "+chinese_number(money)+"两银子 购入"+NOR+query("npaimai/" + sprintf("%d", num))+HIG"的所有权。");
	...
	if( mapp(fpaimai) &&  sizeof(fpaimai) )
	{
		...
		for(i=0;i<sizeof(terms);i++)
		{
			if (ipaimai[terms[i]]==this_player()->query("id") && lpaimai[terms[i]]==0)
			{
				...
				if(!ob){
				 tell_object(ob,"你拍卖的东西不在了，请报告给巫师。\n");
				 return 1;
				}
