...
边的墙上挂着一个牌子，上面写着“巽”。四下望去，旁边好象还有几
...
int do_move()
{
  object ob;
  ob=this_player();
  if(!ob->query_temp("dtsl_lev1_quest_chanzhang/step5"))
  return notify_fail("你使足了力气也推不动把手。\n");
  ob->delete_temp("dtsl_lev1_quest_chanzhang/step5");
  ob->set_temp("dtsl_lev1_quest_chanzhang/step6",1);
