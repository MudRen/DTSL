...
void create()
{
   set_name("×óÓÎÏÉ",({ "zuo youxian","zuo"}) );
        set("gender", "ÄÐÐÔ" );
        set("age", 42);
        set("str",30);
        set("title",HIR"×ÓÎç½£"NOR);
        set("long","Ëû¾ÍÊÇ×ÓÎç½£×óÓÎÏÉ£¬³äÂúºÝÐ°ÎÞÇéµÄÎ¶µÀ\n");
        ...
        set_skill("sword",170);
        set_skill("ziwu-jian",170);
        set_skill("babu-ganchan",170);
    setup();
   carry_object(__DIR__"obj/ziwujian")->wield();
...
void die()
{
  ...
  if(!ob->query_temp("kill_man1"))
   ob->set_temp("kill_man1","×óÓÎÏÉ");
  else if(!ob->query_temp("kill_man2"))
   ob->set_temp("kill_man2","×óÓÎÏÉ");
  else if(!ob->query_temp("kill_man3"))
   ob->set_temp("kill_man3","×óÓÎÏÉ");
  else if(!ob->query_temp("kill_man4")){
   
   ob->set_temp("kill_man4","×óÓÎÏÉ");
   if(ob->query("kar")<=15){
