...
void create()
{
   ...
   set("gender", "男性" );
   set("age",29);
   set("long", "他就是宋家堡的宋爽，是宋缺的本家，他的武功也是不俗。\n");
...
{   
	...
	if(ob->query("office_number")<30){
        ...
        if(!ob->query_temp("bai_songshuang/action2",1)){
          command("say 你没经过我的考验，我如何知道你的胆量？");
          return;
        }
        command("say 你能为宋家的荣誉考虑，已经十分不容易了！好吧，我就收你为徒！");
        
        lev=ob->query_skill("cuff",1);
        lev=lev*lev/(lev+100);
        ob->party_reward_skill("sword",lev);
        tell_object(ob,HIY"你从基本拳法中领略到了基本剑法！\n"NOR);
...
int do_action1()
{
  ...
  if(ob->query_temp("bai_songshuang/action1")){
    command("say 你到底有没有胆量去？");
    return 1;
  }
  if(ob->query_temp("bai_songshuang/action2")){
    command("say 既然你经过了考验，就拜我为师吧！");
    return 1;
  }
  ob->set_temp("bai_songshuang/action1",1);
  target=new(__DIR__"pantu");
  ...
  command("say 好吧！我就看看你的胆量到底如何！\n"+
          "宋家堡有一个叛徒"+target->name()+"在"+MISC_D->find_place(environment(target))+"躲藏，\n"+
          "你去将他正法吧！");
  return 1;
}