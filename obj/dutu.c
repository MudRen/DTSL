...
if(!me->query("pker")) add_action("do_ya", "yabao"); 
...
for (k=0; k<3; k++) say(point[i][k*11..k*11+9]+point[j][k*11..k*11+10]); 
...
"yabao tc <数量> 押头彩(两数顺序及点数均正确) 一赔三十五\n" 

"yabao dc <数量> 押大彩(两数点数正确) 一赔十七\n" 

"yabao sd <数量> 押双对(两数相同且均为偶数) 一赔十一\n" 

"yabao kp <数量> 押空盘(两数不同且均为偶数) 一赔五\n" 

"yabao qx <数量> 押七星(两数之和为七) 一赔五\n" 

"yabao dd <数量> 押单对(两数均为奇数) 一赔三\n" 

"yabao sx <数量> 押散星(两数之和为三、五、九、十一) 一赔二\n" 

"每盘按从上到下的顺序只出现一种点型(头彩和大彩可同时出现)，其他情况都算庄家赢。\n"); 

if (undefinedp(t[type])) 
...
if (mount > 50000000) return notify_fail("您押的数目太大了。\n"); 

if (mount > me->query("deposit")) return notify_fail("您没有这么多存款。\n"); 
...
say(name()+"喊道：本盘头彩骰号是" HIW +chinese_number(tc1)+ NOR "、" HIW +chinese_number(tc2)+ NOR "，各位客倌快押(yabao)啊！\n"); 

if (debug) message("wizard:haozhi", sprintf("%O\n", query("total")), environment()); 
...
if (!me || !present(me, environment())) continue; // 不在房间的不给钱 
...
if (debug) message("wizard:haozhi", "赌场情况："+cc+" "+tt+"。\n", environment()); 
