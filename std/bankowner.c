...
int do_deposit(string arg)
{
    ...
    message_vision(sprintf("$N存入了%s%s%s。\n",
                           chinese_number(amount), type_ob->query("base_unit"), type_ob->query("name")),
                   this_player());
    this_player()->start_busy(1);
    return 1;
}
int do_check(string arg)
{
  ...
  tell_object(this_player(),"你现在共有存款"+MONEY_D->money_str(this_player()->query("deposit"))+"\n");
  return 1;
}
...
int do_withdraw(string arg)
{
    ...
    tell_object(this_player(), sprintf("你取出了%d两%s\n", i, str));
    this_player()->start_busy(1);
    return 1;
}
