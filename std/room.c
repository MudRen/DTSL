...
//        if (query("no_reset")) return;
    ...
    if(!inv[i]->clean_up()) continue;
        destruct(inv[i]);
    }
  //maybe this is a bug ,hehe :-)
  //
    //
    ...
    for(i=0; i<sizeof(list); i++) {
        ...
        case 1:
            ...
            //if food,make another one
            if( environment(ob[list[i]]) != this_object()
                && ob[list[i]]->query("food_remaining"))
        ...
        default:
            for(j=0; j<ob_list[list[i]]; j++) {
                ...
                && ob[list[i]][j]->query("food_remaining"))
                    make_inventory(list[i]);
...
/*void reset()
{
    string dir;
    int i;
    object daemon,map_system;
    mapping *maps=({}),*new_maps=({});

    dir = base_name(this_object());

    i = strsrch(dir,"/",-1);
    dir = dir[0..i-1];
    i = strsrch(dir,"/",-1);
    dir = dir[i+1..strlen(dir)];

    daemon = load_object("/obj/citydaemon/city/"+dir);
    if(!objectp(daemon))
        return;

    maps = daemon->query_citymap();
    for(i=0;i<sizeof(maps);i++){
        if(maps[i]["dir"]==base_name(this_object())+".c"){
            new_maps = new_maps+({ maps[i] });
        }
    }
    map_system = new("/obj/citydaemon/citydaemon");
    map_system->set_city_map(new_maps);
    for(i=0;i<sizeof(new_maps);i++)
        map_system->DoResetMap(i);
    destruct(map_system);
}*/
