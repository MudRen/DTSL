...
// 	int mark_loaded;
	...
	string dir;
       ...
       int opt_l;
	mixed *file;
       string result,wiz_status;

//        if( me->query("id") != "gump" && me->query("id") != "pkyou" && me->query("id") != "fall")
//         return notify_fail(CYN"你没有权利使用这个指令！\n"NOR); 
    ...
    // CHANNEL_D->do_channel(me, "adm","正在LS目录："+ dir +"\n"NOR);
     log_file("static/ls",sprintf("%s(%s)用ls指令查看过 %s 时间在: %s \n",
	me->name(1),	geteuid(me),	dir,	ctime(time()) ) ); 
