...
int main(object me, string arg)
{
    ...
    if(userp(obj) && geteuid()!=ROOT_UID) return notify_fail("你不能摧毁其他的使用者。\n");
	if(objectp(obj) && environment(me)==environment(obj) ){
		if( !stringp(msg = me->query("env/msg_dest")) ) msg = "$N召唤出一个黑洞，将$n吞没了。\n";
		message_vision(msg + "\n", me, obj);
	}
	destruct(obj);
