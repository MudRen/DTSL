...
if( !wizardp(me) && userp(env) ) {
	...
	if( env ->query_temp("netdead") )
                return notify_fail("你不能这样无耻的拿断线的人的东西.\n");
       }
	...
	if(arg=="all") {
		...
		if( !env->query_max_encumbrance() )	return notify_fail("那不是容器。\n");
                if( env->query("no_get_from") )	return notify_fail("你不能从那里面拿出东西来。\n");
...
int do_get(object me, object obj)
{
	...
	if( !wizardp(me) && 
     ...
     return notify_fail("你身上的东西太多了，不能拣起"+obj->name()+"。\n");
