...
void done_post(object me, mapping note, string text)
{
    ...
    if( !pointerp(notes) || i > 0 )
        ...
        notes += ({ note });
    tell_object(me, HIM "求救OK。请等待。\n");
 // tell_object(me, HIM "巫师正忙着，请等待。\n");
 // tell_object(me, HIM "你先做你自己的事情吧，一会用sos read看看是否解决了，ｏｋ？\n" NOR);
	message("channel:wiz", HIY "【求救】：玩家 "+note["author"]+" 使用"BLINK" SOS "NOR HIY"紧急求救，请在线巫师使用 sos 命令查看。\n" NOR,users());
    save();
    return;
}
...
int main(object me, string arg)
{
    ...
    if (!arg) {
        if(!wiz_level(me)){
            tell_object(me, HIM "你现在是在" HIY MUD_NAME HIM "里，你可以向在线WIZ求救。\n" NOR);
      //    tell_object(me, HIM "赶快向巫师们发出求救：ＳＯＳ！\n" NOR);
