...
void ready_to_start()
{
        ...
        remove_call_out("check_start_story");
        call_out("check_start_story",60);//间隔时间
//        call_out("start_story", 1 + random(1));//间隔时间
}

void check_start_story()
{
   times++;
   if(times>=30){
    times=0;
    call_out("start_story", 1 + random(1));
   }
   else{
    remove_call_out("check_start_story");
    call_out("check_start_story",60);
   }
}
