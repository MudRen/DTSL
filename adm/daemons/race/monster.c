...
void setup_monster(object ob)
{
        ...
        if(undefinedp(my["str"])) my["str"] = random(31) + 15;
        if(undefinedp(my["int"])) my["int"] = random(31) + 15;
        if(undefinedp(my["per"])) my["per"] = random(31) + 15;
        if(undefinedp(my["con"])) my["con"] = random(31) + 15;
        if(undefinedp(my["con"])) my["cor"] = random(31) + 15;

        if(undefinedp(my["max_gin"])) 
        {
                if(my["age"] <= 3) my["max_gin"] = 50;
                else if(my["age"] <= 10) 
                        my["max_gin"] = 50 + (my["age"] - 3) * 30;
                else if(my["age"] <= 60) 
                        my["max_gin"] = 260 + (my["age"] - 10) * 5;
        ...
        //      NOTE: monster has no initial limbs defined, you must 
        //      define it yourself. ob->init_limbs(LIMBS);
        if(!ob->query_weight()) 
