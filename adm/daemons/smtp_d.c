...
protected string mailname = "dtsl",mailpasswd = "这个不能告诉你";
...
protected void success_register(int fd)
{
	...
	(mailmsg->user)->stop_busy();
	map_delete(content,fd);
	tell_object(usr,sprintf("给您分配的随机密码已根据您登记的地址发往：%s
