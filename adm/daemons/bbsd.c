...
int add_Bbs_Up_Map(string db,string sql,object ob,string fun,int forbid)
{
	...
	//自己呼叫	
	if(!test_debug && INTERMUD_MUD_NAME=="SJ" 
	&& previous_object()
	...
	if (fd<0) {
		log_file("static/bbs", sprintf("error   :%15s:%5d:\n","socket_create",fd), 0);
		return 0;
	}
...
void write_callback2(int fd)
{
	...
	if ( result != EESUCCESS ) {
		log_file("static/bbs", sprintf("%-15serror: 目标数据库:%-8s提交时间：%sSQL操作:%s","socket_write",Bbs_Up_Map[fd]["database"],ctime(Bbs_Up_Map[fd]["time"]),Bbs_Up_Map[fd]["sql"]),0);
		over_set(fd);
		return;
	}
}
...
static mapping* bbs_info;
static int in_work=0;
static int this_post_is_not_reaply;
...
int bbs_post(int fid,string author,string subject,string message,int tid,int topped,int digest,object ob,string fun)
{
	...
	if(!test_debug && INTERMUD_MUD_NAME=="TEST" 
	&& previous_object()!=find_object("/cmds/usr/news.c") 
	&& previous_object()!=find_object("/cmds/arch/newucp.c")
	&& previous_object()!=find_object("/cmds/adm/bbs.c")
	) return 0;
