...
int execute(object me,object victim)
{
  ...
  if(me->query_skill("busi-yin",1))
  return notify_fail("你学了不死印，无法施展惜花！\n");
  ...
  if(time()-me->query_temp("execute_busy")<15)
  return notify_fail("你不能连续使用心法。\n");
  
  lev=me->query_skill("huachan",1);
  ...
  msg=HIM"$N"+HIM+"手指微动，似乎在抚摩着花朵，周身立刻产生一股祥和的气势！\n"NOR;
