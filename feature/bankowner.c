...
int do_convert(string arg)
{
	...
	if( !bv1 ) return notify_fail("这种东西不值钱。\n");
        if(from==to)
        return notify_fail(this_object()->query("name")+"眼睛一瞪：怎么？想捣乱吗？！\n");
...
int do_check(string arg)
{
	...
	tell_object(this_player(),"你现在共有存款"+MONEY_D->money_str(value)+"\n");
	return 1;
}
...
int do_withdraw(string arg)
{
	...
	if(str=="silver") str="白银";
	else if(str=="gold") str="黄金";
	else str="铜钱"; 
	hamount -=amount;
	this_player()->set("deposit", hamount);
	pay_player(this_player(), amount);
	if(str!="铜钱")
	tell_object(this_player(),"你取出了"+chinese_number(i)+"两"+str+"\n");
	else tell_object(this_player(),"你取出了"+chinese_number(i)+"文"+str+"\n");
	this_player()->start_busy(1);
	return 1;
