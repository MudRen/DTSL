...
private void _del_char(object pl, int bs)                // 删除游标上的字
{
        ...
        else        go_left = 1;                // 因为是删除行尾的字，所以要左移游标
...
{
        printf(REF +
"【 MudOS 超级LPC编辑器 】 ver: 0.9a  by 东西方不败(lanxin)  \r
\r
...
注意:本版的编辑器请使用Crt 或 NetTerm之类的TELNET程序,暂时不兼容zmud
      类的MUD专用程序!
