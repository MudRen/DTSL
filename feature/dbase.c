...
mixed add_array(string name,string str)
{
  string *org;
  org=query(name);
  if(!org)
  return set(name,({str}));
  if(sizeof(org)>sizeof(org-({str})))
  return set(name,org);
  return set(name,org+({str}));
}

mixed add_temp_array(string name,string str)
{
  string *org;
  org=query_temp(name);
  if(!org)
  return set_temp(name,({str}));
  if(sizeof(org)>sizeof(org-({str})))
  return set_temp(name,org);
  return set_temp(name,org+({str}));
}

mixed reduce_array(string name,string str)
{
  string *org;
  org=query(name);
  if(!org)
  ...
  if(sizeof(org)>sizeof(org-({str})))
   set(name,org-({str}));
 org=query(name);
  if(!sizeof(org)) delete(name);
}
mixed reduce_temp_array(string name,string str)
{
  string *org;
  org=query_temp(name);
  if(!org)
  return delete_temp(name);
  if(sizeof(org)>sizeof(org-({str})))
   set_temp(name,org-({str}));
  org=query_temp(name);
  if(!sizeof(org)) delete_temp(name);
}
